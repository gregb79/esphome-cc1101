...
void CC1101::get_info()
{
  ...
  uint32_t freq_hw = static_cast<uint32_t>(frequency * 1000);  // return in KHz
  
  // modulation 
  uint8_t mdmcfg2 = this->read_config_register(CC1101_MDMCFG2);
  int mod_hw = (mdmcfg2 & 0x70) >> 4;
  ...
  float dev_hw = (pow(2, e) * (8 + m) * 625.0f) / 1000.0f;  // return in kHz
  
  // display all
  const char *mod_names[] = {"2-FSK", "GFSK", "ASK/OOK", "4-FSK", "MSK"};

  ESP_LOGI(TAG, "Current CC1101 Values:");
